...
def EdAlign_loss(model, X, y):
    ...
    euclidean_dist_diff  = tc.sum((grad1_normed - grad2_normed) ** 2, dim=1)  # 计算差的平方和  
    euclidean_dist = tc.sqrt(euclidean_dist_diff)  # 取平方根得到欧几里得距离  
...
transform_to_224 = transforms.Compose([  
    ...
    transforms.ToPILImage(),  # 将Tensor转换为PIL.Image，这一步在save_image中其实不是必需的，但有助于理解  
]) 
...
def svhn_dataloaders(batch_size=128,num_workers=2, data_dir = '/root/WYHH/data/svhn'):
...
def get_datasets(args):
    if args.datasets == 'CIFAR10':
        ...
        return stl10_dataloaders(train_batch_size=args.batch_size, num_workers=args.workers)
...
def stl10_dataloaders(train_batch_size=64, test_batch_size=100, num_workers=2, data_dir = '/root/WYHH/data/stl10'):

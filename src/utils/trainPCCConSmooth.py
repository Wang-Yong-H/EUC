...
def train1(args):
    ...
    with open("PGDConSmooth.txt", "w") as f:
...
def update(loader, model, criterion, optimizer, epoch, args, swa_model):
    ...
    for i, (img, tgt) in enumerate(loader, 1):
        ...
        label_smoothing = Variable(torch.tensor(_label_smoothing(tgt, args.factor, num_classes)).cuda()).float()
        img.requires_grad_(True)
        opt = model(img)
        ori_loss =  LabelSmoothLoss(opt, label_smoothing.float())# F.cross_entropy(ori_output, y)
        
        ig = grad(ori_loss, img)[0]
        ig_norm = tc.norm(ig, p=1)
        ...
        # loss_reg1 = 0
        loss_reg2 = 0
        loss_reg2 = torch.mean(torch.sign(opt2.float() - opt.float()) * torch.sign(opt.float()))

        loss = LabelSmoothLoss2(args, loss_reg2,opt2, label_smoothing.float())
...
def LabelSmoothLoss(input, target):
    ...
    return loss
def LabelSmoothLoss2(args, loss_reg2,input, target):
    log_prob = F.log_softmax(input, dim=-1)
    loss = (-target * log_prob).sum(dim=-1).mean()
    return loss

...
def train1(args):
    ...
    pre_loss = float('inf')  
    strength=0.8
    with open("NWPU18PGD_EL.txt", "w") as f:
     ...
     for epoch in range(start_epoch, args.epochs):
        ...
        update(train_loader, model, criterion, optimizer, epoch, args, swa_model,pre_loss,strength)
...
def PCC_Loss(opt2, img,model,target,strength):  
    smoothing=0.1
    log_prob = F.log_softmax(opt2, dim=-1)  
    num_classes = log_prob.size(-1)  
    # 创建平滑后的标签  
    ...
    loss = ((-smoothed_target * log_prob).sum(dim=-1) + strength * EdAlign_loss(model, img, target)).mean()#正则
    return loss    
...
def update(loader, model, criterion, optimizer, epoch, args, swa_model,pre_loss,strength):
    ...
    for i, (img, tgt) in enumerate(loader, 1):
        ...
        opt = model(adv)
        # loss = criterion(opt, tgt)
        loss = PCC_Loss(opt, img,model,tgt,strength)
        if loss > pre_loss:
          strength=1.1
        else:
          strength=0.9
...
#     diff_sq = tc.sum((grad1_normed - grad2_normed) ** 2, dim=1)  
#     reg = 0.2 * diff_sq.mean() 
...
def EdAlign_loss(model, X, y):
    ...
    diff_sq = tc.sum((grad1_normed - grad2_normed) ** 2, dim=1)  # 计算差的平方和  
    euclidean_dist_sq = diff_sq  # 欧几里得距离的平方  
    euclidean_dist = tc.sqrt(euclidean_dist_sq)  # 取平方根得到欧几里得距离  
    reg = 0.2 * euclidean_dist.mean()  # 计算欧几里得距离的平均值并乘以常数
    return reg
"执行了以下步骤来度量两个梯度向量集之间的“对齐”程度，但实际上是以欧几里得距离的平均值来衡量的"
...
def nwpu_resisc45_dataloaders(batch_size=64, num_workers=2):  
  ...
  # 加载数据集
    train_set = NWPU_RESISC45(root_dir='/root/WYH/project/data/NWPU/train', train=True, transform=train_transform)
# 获取训练集的长度
...
# 加载测试集
    test_set = NWPU_RESISC45(root_dir='/root/WYH/project/data/NWPU/test', train=False, transform=test_transform)
# 创建数据加载器

...
from src.utils.adversary import pgd, perturb,ni_fgsm
...
def validate2(loader, model, criterion, args):
    batch_time = AverageMeter('Time', ':4.3f')
    losses = AverageMeter('Loss', ':.2e')
    acc1 = AverageMeter('Acc@1', ':6.2f')
    ign = AverageMeter('InGrad', ':.2e')
    fgsm = AverageMeter('FGSM', ':6.2f')
    nifgsm = AverageMeter('nifgsm', ':6.2f')
    meters = [batch_time, losses, ign, acc1, fgsm,nifgsm]
    progress = ProgressMeter(len(loader), meters, prefix='Test: ')
    ...
    for i, (img, targets) in enumerate(loader, 1):
        ...
        if args.eval_iter > 0:
            ...
            adv, _ = ni_fgsm(img, targets, model, criterion, args.eps, mu=0.0, noise_std=0.1, num_noise=10)
            ni_fgsm_accuracy = accuracy(model(adv), targets)[0][0]
            nifgsm.update(ni_fgsm_accuracy, bs)
    ...
    sprint(' *  Acc@1: {0.avg:.2f} | FGSM: {1.avg:.2f} | nifgsm: {2.avg:.2f} | IG: {3.avg:.3e}'
           .format(acc1, fgsm,nifgsm, ign))
    return acc1.avg.item(), ign.avg.item(), fgsm.avg.item(),nifgsm.avg.item()

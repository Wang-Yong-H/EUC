...
PATH.MODEL = os.path.join(root_path, 'modelAT')
...
def get_available_cudas():
    if tc.cuda.is_available():        
        pynvml.nvmlInit()
        ncudas = pynvml.nvmlDeviceGetCount()
        acudas = []
        for i in range(ncudas):
            cuda = pynvml.nvmlDeviceGetHandleByIndex(i)
            procs = pynvml.nvmlDeviceGetComputeRunningProcesses(cuda)
            # if procs == []:
            acudas.append(i)
        pynvml.nvmlShutdown()
        return acudas
...
def get_memory_info(cuda_idx):
    ...
    pynvml.nvmlInit()
    for cid in cuda_idx:
        cuda =pynvml.nvmlDeviceGetHandleByIndex(cid)
        mem_info = pynvml.nvmlDeviceGetMemoryInfo(cuda)
